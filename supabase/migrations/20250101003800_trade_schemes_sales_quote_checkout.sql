...
BEGIN
  ...
  order_id := public.create_sales_order_for_customer(p_customer_id, p_lines, p_notes, p_payment_method);
  FOR scheme_row IN
...
BEGIN
  ...
  SELECT o.customer_id INTO customer_id FROM public.orders o WHERE o.id = order_id;
  IF customer_id IS NULL THEN RAISE EXCEPTION 'The accepted quote did not create a valid customer order'; END IF;
  FOR scheme_row IN
    SELECT s.id, s.is_stackable, s.priority
    FROM public.schemes s
    WHERE s.is_active AND now() >= s.starts_at AND now() < s.ends_at
      AND (s.audience_type = 'ALL' OR EXISTS (SELECT 1 FROM public.scheme_audiences sa WHERE sa.scheme_id = s.id AND (sa.customer_id = customer_id OR sa.vendor_group_id = (SELECT vendor_group_id FROM public.customers WHERE id = customer_id))))
      AND (s.is_stackable OR NOT applied_non_stackable)
    ORDER BY s.is_stackable DESC, s.priority ASC, s.id
  LOOP
    IF public.apply_trade_scheme_to_order(scheme_row.id, order_id, customer_id) IS NOT NULL AND NOT scheme_row.is_stackable THEN applied_non_stackable := true; END IF;
  END LOOP;
  RETURN order_id;
END;
$$;
