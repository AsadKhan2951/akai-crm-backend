...
BEGIN
  ...
  END IF;
  SELECT c.customer_id INTO customer_id FROM public.carts c WHERE c.id = p_cart_id AND c.status = 'ACTIVE' FOR UPDATE;
  IF customer_id IS NULL THEN RAISE EXCEPTION 'The active cart could not be found'; END IF;
  ...
  FOR scheme_row IN
    ...
    WHERE s.is_active AND now() >= s.starts_at AND now() < s.ends_at
      AND (s.audience_type = 'ALL'
        OR EXISTS (SELECT 1 FROM public.scheme_audiences sa WHERE sa.scheme_id = s.id AND (sa.customer_id = customer_id OR sa.vendor_group_id = (SELECT vendor_group_id FROM public.customers WHERE id = customer_id))))
  ...
  LOOP
    IF scheme_row.is_stackable OR NOT applied_non_stackable THEN
      IF public.apply_trade_scheme_to_order(scheme_row.id, order_id, customer_id) IS NOT NULL AND NOT scheme_row.is_stackable THEN
