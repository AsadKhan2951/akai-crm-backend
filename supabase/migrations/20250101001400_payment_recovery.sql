...
returns table(bucket text, customer_count integer, receivable_pkr numeric(12,2)) language sql stable security invoker set search_path=public as $$
  select case when greatest(0,current_date-coalesce(last_payment_at::date,max(o.placed_at)::date,current_date)) <=30 then '0-30' when greatest(0,current_date-coalesce(last_payment_at::date,max(o.placed_at)::date,current_date)) <=60 then '31-60' when greatest(0,current_date-coalesce(last_payment_at::date,max(o.placed_at)::date,current_date)) <=90 then '61-90' else '90+' end, count(*)::integer, sum(greatest(c.current_balance_pkr,0))::numeric(12,2) from public.customers c left join lateral (select max(pc.cleared_at) last_payment_at from public.payment_collections pc where pc.customer_id=c.id and pc.status='CLEARED') p on true left join public.orders o on o.customer_id=c.id and o.status <> 'CANCELLED' where public.has_permission(auth.uid(),'collection.view') and public.has_permission(auth.uid(),'financials.view_revenue') and c.is_internal_account=false and c.current_balance_pkr>0 group by 1 order by case when 1=1 then 1 end;
$$;
