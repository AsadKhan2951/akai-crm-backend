...
- `20250101006600_performance_and_admin_summary.sql`: speed pass. Indexes every foreign key, makes RLS permission checks run once per query instead of once per row, and adds `current_user_context()` (profile, role and permissions in one call), `admin_ops_summary()` and `admin_nav_badges()` for the new Admin console, and `admin_bulk_update_customers()` (bulk agent/type changes with an audit row).
